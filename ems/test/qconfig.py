...
#        self.assertRaises(TypeError,c.setProfile,'myprofile', {})
    ...
    def testEmitting(self):
        ...
        t.connect(c,
          SIGNAL("entryChanged(QString,QString,QString)"),
          t.receiveEdit)
        t.connect(c,
                  SIGNAL("entryDeleted(QString,QString)"),
                  t.receiveDelete)
        t.connect(c,
                  SIGNAL("profileChanged(QString)"),
                  t.receivePChange)
        t.connect(c,
                  SIGNAL("profileDeleted(QString)"),
                  t.receivePDelete)
        t.connect(c,
                  SIGNAL("standardProfileChanged(QString)"),
                  t.receiveSPChange)
        t.connect(c,
                  SIGNAL("configLoaded(QString)"),
                  t.receiveLoaded)
        t.connect(c,
                  SIGNAL("configSaved(QString)"),
                  t.receiveSaved)
        t.connect(c,
                  SIGNAL("profileNameChanged(QString,QString)"),
                  t.receivePNameChanged)
        c.setProfile('first', Node())
        c.setProfileName('first', 'First Profile')
